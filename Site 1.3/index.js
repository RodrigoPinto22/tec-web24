...
//ALTEREI ESTA PARTE PARA COMEÇAR NA COR ESCOLHIDA, MAS A IMPLEMENTACAO ESTÁ ERRADA NA FUNCAO PLACE_PIECES
let counter=0;
let corcomeca=0;
function corinicial(){
...
function placePieces(event) {
    ...
    //POR CAUSA DESTE COUNTER A IMPLEMENTACAO QUE PUS ACIMA ESTÁ ERRADA
    if (counter === 10) {
        const corners = document.getElementsByClassName("corner");
        const sides = document.getElementsByClassName("side");
        for (let i = 0; i < corners.length; i++) {
...
//ESTA FUNÇÃO ESTÁ ERRADA PORQUE NÃO DÁ BEM O RESET
function resetGame() {
    counter=corcomeca;
    red_cells = [];
    blue_cells = [];
    ...
    // Resetar células, removendo seleções e outras classes, caso existam
    const cells = document.querySelectorAll(".cell");
    cells.forEach(cell => {
        cell.classList.remove("selected");  // Caso tenha células selecionadas
        cell.classList.remove("occupied"); // Caso você tenha uma classe para indicar células ocupadas
    });

    start_game();
}

// Associando o botão de reset à função
document.getElementById("resetButton").addEventListener("click", resetGame);

// A função start_game() e o restante do código permanecem os mesmos
